print("Starting script...")  # ← ये print होगा, अगर नहीं होता तो Python issue है
...
import PyPDF2
from concurrent.futures import ProcessPoolExecutor, as_completed  # ← Multiprocessing के लिए
import threading
...
TELEGRAM_BOT_TOKEN = "8876830548:AAEBRWb2L_nU88UUfmkzBe281R0xVOrnDmw"   # <--- fixed
...
# ============== PDF PASSWORD CRACKER (OPTIMIZED) ==============
class PDFPasswordCracker:
    def __init__(self):
        # अब __init__ में कोई executor नहीं बनाते – crack_pdf() में बनेगा
    ...
    def _try_password(pdf_bytes, password):
        """PDF bytes और एक password लेकर decrypt attempt करता है – static method ताकि pickle हो सके"""
        try:
            from io import BytesIO
    ...
    def crack_pdf(self, pdf_path, name, progress_callback=None):
        ...
        # ---- PDF को मेमोरी में पढ़ें (एक बार) ----
        with open(pdf_path, 'rb') as f:
            pdf_bytes = f.read()

        # ---- पासवर्ड पैटर्न जनरेट करें (वही जो पहले था) ----
        ...
        # YEARS: नवीनतम से पुराने की ओर (2026 ... 1930)
        ...
        # ---- MULTIPROCESSING CRACKING (Executor crack_pdf के अंदर) ----
        # CPU कोर की संख्या के अनुसार workers
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i, pwd in enumerate(unique_passwords):
                if self.stop_flag:
                    break
                # submit task – static method का reference
                futures.append(executor.submit(self._try_password, pdf_bytes, pwd))
                # जब batch full हो या आखिरी password हो तो results collect करें
                if len(futures) >= batch_size or i == total_passwords - 1:
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            continue
                    futures = []  # completed futures हटाएँ

        # ---- Fallback: बिना year वाले patterns (जैसे "RAJE") ----
        if not self.stop_flag:
            ...
            for password in no_year_passwords:
                if self.stop_flag:
                    break
                success, found_pwd = self._try_password(pdf_bytes, password)  # सीधे call (single process)
                if success:
                    self.found_password = found_pwd
                    self.stop_flag = True
                    decrypted_path = self.decrypt_pdf(pdf_path, found_pwd)
...
class AadhaarBot:
    ...
    # ... (बाकी सभी methods बिल्कुल वही रहेंगे – कोई बदलाव नहीं)
...
BOT_NAME        = "✜ Uɪᴅᴀɪ-ɢʀᴀᴍ"
OWNER_ID        = 8734310359
OWNER_USERNAME  = "@sellermackinnon"
SESSION_TIMEOUT = 60
...
CHANNEL_USERNAME = "@veloxshop24x7"
CHANNEL_LINK     = "https://t.me/veloxshop24x7"
...
def main():
    ...
    try:
        ...
        if bot_info.get('ok'):
            ...
            print(f"[ proxy  ]  {'on' if PROXY_CONFIG['use_proxy'] else 'off'}")
            print(f"[ cracker]  PyPDF2 / multiprocessing (CPU cores: {os.cpu_count()})")
